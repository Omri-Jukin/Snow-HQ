...
// tRPC provider is not present; use a passthrough for now
const TRPCProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => <>{children}</>
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const storedLayout = localStorage.getItem('layout') as TResponsiveLayout | null
    if (storedLayout) {
  ...
  const baseTheme = createAppTheme({ mode: isDarkMode ? 'dark' : 'light' })
